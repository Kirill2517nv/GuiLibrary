...
// Одинаковый масштаб по обеим осям: метр по горизонтали и метр по вертикали
// занимают одинаковое число пикселей. Нужно там, где график – это картина
// происходящего, а не зависимость одной величины от другой: маятник на
// стержне, орбита, поле частиц. Без этого окно шириной больше высоты
// растягивает картинку, и круговое движение выглядит эллипсом.
//
// Границы осей при этом остаются вашими: лишнее место ImPlot добавит сам
// с той стороны, где его не хватает.
void set_plot_equal_axes(const std::string& name, bool equal = true);

// Квадратное полотно: график занимает не всё окно, а квадрат со стороной по
// меньшему из размеров и стоит посередине. Имеет смысл при размере (-1, -1),
// когда полотно тянется за окном: в широком окне график расползается в
// полосу, и на нём трудно что-либо разглядеть.
void set_plot_square(const std::string& name, bool square = true);
