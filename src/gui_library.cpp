...
#else
    // Темп задаёт вертикальная синхронизация: glfwSwapInterval(1) в
    // init_gui_library заставляет glfwSwapBuffers ждать следующего обновления
    // монитора. То есть частота кадров равна частоте монитора – 60 Гц у
    // обычного, 144 у игрового, и никаких «ровно 60» в коде нет.
    //
    // Раньше здесь дополнительно стоял sleep_ms(16). Вместе с ожиданием
    // синхронизации кадр выходил длиннее периода обновления, свежий кадр не
    // успевал к развёртке и ждал следующей – на 60-герцовом мониторе расчёт
    // шёл примерно вдвое медленнее, чем мог.
    //
    // ponytail: если драйвер игнорирует вертикальную синхронизацию, цикл
    // закрутится на полной скорости и займёт ядро целиком. Появится такая
    // машина – вернуть ограничение по времени кадра, а не глухой sleep.
    while (gui_main_loop()) {
    }
