...
int main() {
    ...
    // растягивается по ширине браузера, полотно в пикселях под это не подстроить.
    create_plot("Колебания", 0.f, 200.f, -2.f, 2.f, -1, -1);
    set_plot_axes("Колебания", "t, с", "y, м");

    set_calculation_function(calculation_function);

    // Пульт слева, график справа. 0.4 вместо 0.25 по умолчанию: поле ввода в
    // пульте занимает 150 px, и в более узкой панели подпись «Шаг по времени»
    // не помещается. Без этой строки окна всплывают свободно и налезают друг
    // на друга – так и было до задачи про подвижную мишень.
