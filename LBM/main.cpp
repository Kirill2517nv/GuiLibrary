...
void calculation_function() {
    ...
    clear_plot("K(phi)");
    {
        ...
        for (int i = 0; i < N_pts; i++) {
            ...
            phi_kc[i] = phi;
            // Козени–Карман для ДВУМЕРНОЙ среды: K = A · φ^2 / (1 - φ).
            //
            // Знаменатель первой степени - не описка. Учебная формула
            // φ^3/(1-φ)^2 выведена для зёрен постоянного размера, у которых
            // удельная поверхность от пористости не зависит. Здесь число
            // цилиндров закреплено, а пористость меняют радиусом, и он входит
            // в ответ: R_h = φL²/(2πNr) при N r² = (1-φ)L²/π даёт K ~ R_h²  ~
            // φ^2/(1-φ). В трёх измерениях та же подстановка (r ~ (1-φ)^(1/3)
            // для сферы) дала бы (1-φ)^(4/3) - показатель зависит от
            // размерности, как только зёрна меняют размер.
...
int main() {
    ...
    // --- Подгоночные параметры Козени–Кармана: K = A · φ^2 / (1−φ) ---
