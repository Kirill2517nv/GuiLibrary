...
int main() {
    ...
    // Размер (-1, -1) – «занять всё доступное место»: кадр на странице
    // растягивается по ширине браузера, полотно в пикселях под это не
    // подстроить. Раньше здесь стояло 700 x 700, и в кадре пониже орбита
    // просто не помещалась целиком.
    create_plot("Гравитация", -3.f, 3.f, -3.f, 3.f, -1, -1);
    set_plot_axes("Гравитация", "x, безразм.", "y, безразм.");
    // Орбита – картина происходящего, а не зависимость одной величины от
    // другой: единица длины по горизонтали и по вертикали обязана занимать
    // одинаковое число пикселей, иначе круговая орбита выглядит эллипсом и
    // первую космическую скорость не подобрать на глаз. Квадратное полотно
    // держит пропорцию само, флаг равных осей – ещё и при зуме мышью.
    ...
    // 0.4 вместо 0.25 по умолчанию: поле ввода в пульте занимает 150 px, и в
    // более узкой панели подписи «Начальная скорость Vy» и «alpha = G*M»
